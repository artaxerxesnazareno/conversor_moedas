...
import 'package:conversor_moedas/infrastructure/adapters/http_adapter/https_client.dart';
import 'package:http/http.dart' as http;
import 'package:conversor_moedas/core/config/config.dart';
...
class HttpAdapter extends IHttpClient {
  ...
  Future<HttpResponse> post(String url, dynamic body) async {
    final response = await http.post(Uri.parse('${ApiConfig.baseUrl}$url'), body: jsonEncode(body));
  ...
  Future<HttpResponse> put(String url, dynamic body) async {
    final response = await http.put(Uri.parse('${ApiConfig.baseUrl}$url'), body: jsonEncode(body));
  ...
  Future<HttpResponse> patch(String url, dynamic body) async {
    final response = await http.patch(Uri.parse('${ApiConfig.baseUrl}$url'), body: jsonEncode(body));
